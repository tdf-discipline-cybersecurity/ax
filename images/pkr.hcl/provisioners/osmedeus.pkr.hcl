  ...
  provisioner "shell" {
    ...
    inline = [
      ...
      "echo 'Remove Azure Agent and Microsoft components from image'",
      "apt purge -y --allow-change-held-packages waagent mdatp auoms",
      "rm -rf /var/lib/waagent /etc/waagent.conf /var/log/waagent.log /opt/microsoft /etc/opt/microsoft",
      ...
      "mkdir -p /home/op/osmedeus-install",
      "curl -fsSL ${var.osmedeus_install_path} -o /home/op/osmedeus-install/osmedeus-install.sh",
      "sed -i 's/\\/tmp\\//\\/home\\/op\\/osmedeus-install\\//g' /home/op/osmedeus-install/osmedeus-install.sh",
      "/bin/su -l root -c 'bash -v /home/op/osmedeus-install/osmedeus-install.sh'",
      "rm -rf /home/op/osmedeus-install",
